...
async fn handle_callers(args: CallersArgs) -> anyhow::Result<()> {
    ...
    if output.status.success() {
        ...
        Err(anyhow::anyhow!("Trace failed: {}", stderr))
    }
}

async fn handle_callees(args: CalleesArgs) -> anyhow::Result<()> {
    use std::process::Command;
...
async fn handle_graph(args: GraphArgs) -> anyhow::Result<()> {
    use std::process::Command;
    ...
    if output.status.success() {
        ...
        let stderr = String::from_utf8_lossy(&output.stderr);
        Err(anyhow::anyhow!("Graph generation failed: {}", stderr))
    }
}
