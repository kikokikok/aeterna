...
use std::sync::Arc;
use tools::memory::{MemoryAddTool, MemorySearchTool, MemoryDeleteTool};
...
async fn test_memory_tools() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    // WHEN adding memory
    let add_resp = add_tool.call(json!({
    ...
    let search_resp = search_tool.call(json!({
    ...
    let delete_resp = delete_tool.call(json!({
        "memory_id": memory_id,
        "layer": "user"
    })).await?;
