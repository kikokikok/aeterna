//! # GrepAI MCP Tool Implementations
//!
//! MCP tools that proxy to GrepAI sidecar for semantic code search and call graph analysis.

use crate::grepai::client::GrepAIClient;
use crate::grepai::types::{CodeChunk, SymbolReference, CallGraphNode, IndexStatus};
...
pub struct CodeSearchTool {
    client: Arc<GrepAIClient>,
}

impl CodeSearchTool {
    pub fn new(client: Arc<GrepAIClient>) -> Self {
        Self { client }
    }
}
...
impl Tool for CodeSearchTool {
    ...
    async fn call(&self, params: Value) -> Result<Value, Box<dyn std::error::Error + Send + Sync>> {
        ...
        // Check if GrepAI is available
        if !self.client.is_available() {
            return Ok(json!({
                "success": false,
                "error": "GrepAI sidecar is currently unavailable",
                "results": []
            }));
        }

        // Prepare GrepAI request
        let grepai_params = json!({
            "query": p.query,
        ...
        // Call GrepAI
        match self.client.call_tool("grepai_search", grepai_params).await {
            Ok(response) => {
                Ok(json!({
                    "success": true,
...
impl Tool for CodeTraceCallersTool {
    ...
    async fn call(&self, params: Value) -> Result<Value, Box<dyn std::error::Error + Send + Sync>> {
        ...
        match self.client.call_tool("grepai_trace_callers", grepai_params).await {
            Ok(response) => Ok(json!({
                "success": true,
...
impl Tool for CodeTraceCalleesTool {
    ...
    async fn call(&self, params: Value) -> Result<Value, Box<dyn std::error::Error + Send + Sync>> {
        ...
        match self.client.call_tool("grepai_trace_callees", grepai_params).await {
            Ok(response) => Ok(json!({
                "success": true,
...
impl Tool for CodeGraphTool {
    ...
    async fn call(&self, params: Value) -> Result<Value, Box<dyn std::error::Error + Send + Sync>> {
        ...
        match self.client.call_tool("grepai_trace_graph", grepai_params).await {
            Ok(response) => Ok(json!({
                "success": true,
...
impl Tool for CodeIndexStatusTool {
    ...
    async fn call(&self, params: Value) -> Result<Value, Box<dyn std::error::Error + Send + Sync>> {
        ...
        match self.client.call_tool("grepai_index_status", grepai_params).await {
            Ok(response) => Ok(json!({
                "success": true,
...
mod tests {
    use super::*;
    use crate::grepai::client::GrepAIConfig;

    #[tokio::test]
    async fn test_code_search_tool() {
        let client = Arc::new(GrepAIClient::new(GrepAIConfig::default()));
    ...
    async fn test_code_trace_callers_tool() {
        ...
        assert_eq!(tool.name(), "code_trace_callers");
        assert!(!tool.description().is_empty());
    }
}
