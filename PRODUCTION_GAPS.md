...
| ID | Gap | Problem | Solution |
|----|-----|---------|----------|
| OC-M1 | **Package naming undefined** | `@aeterna-org/opencode-plugin` vs `aeterna-opencode` not decided. | Standardize on `@aeterna-org/opencode-plugin` for namespace consistency. |
