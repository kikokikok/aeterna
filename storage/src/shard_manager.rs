...
impl ShardManager {
    ...
    pub fn remove_shard(&mut self, shard_id: &str) -> Result<ShardInfo, ShardError> {
        ...
        if shard.current_tenants > 0 {
            ...
            return Err(ShardError::ProvisionFailed(format!(
                "Cannot remove shard {} with {} active tenants",
                shard_id, shard.current_tenants
            )));
        }
    ...
    pub fn get_statistics(&self) -> ShardStatistics {
        ...
        let active_shards = self
            ...
            .count();
        let total_capacity = self.shards.values().map(|s| s.max_capacity).sum();
        let total_tenants = self.shards.values().map(|s| s.current_tenants).sum();
...
mod tests {
    ...
    fn test_increment_decrement_tenant_count() {
        ...
        assert_eq!(
            manager.get_shard("shared-shard-1").unwrap().current_tenants,
            1
        );
    ...
    fn test_shard_lifecycle() {
        ...
        assert_eq!(
            manager.get_shard(&shard_id).unwrap().status,
