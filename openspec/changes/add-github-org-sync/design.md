...
The project already has a working GitHub App (`aeterna-knowledge-bot`, app ID 3191516, installation ID 119155108) with certificate-based authentication for the knowledge repository governance track. The `knowledge` crate's `GitHubProvider` implements JWT → installation token minting via `octocrab` + `jsonwebtoken`, with a `CachedToken` pattern that refreshes 5 minutes before expiry.
