...
- [ ] 2.4 Implement `POST /api/v1/admin/tenants/diff` returning structured delta between submitted manifest and current state. _(Sequenced after full-fidelity renderer: a diff is only trustworthy when every manifest section round-trips. §2.2-A landed `providers` (llm + embedding) forward-apply + reverse-render with secret_ref operator-name recovery; `hierarchy` / `roles` / `providers.memoryLayers` are still forward-path gaps — details in `FINDINGS-2-2.md`. Diff ships after those four sub-tasks close.)_
