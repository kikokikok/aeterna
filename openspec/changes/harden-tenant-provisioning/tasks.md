...
- [ ] 2.4 Implement `POST /api/v1/admin/tenants/diff` returning structured delta between submitted manifest and current state. _(**Unblocked as of §2.2-D (this PR).** A diff is only trustworthy when every manifest section round-trips; that condition is now satisfied. §2.2-A landed `providers` (llm + embedding) forward-apply + reverse-render with secret_ref operator-name recovery. PR #128 landed `tenant.domainMappings` reverse-render and clarified top-level `secrets` as wire-only (not a gap). §2.2-B (PR #143 B3 + PR #148 B4) landed `hierarchy` via `HierarchyStore` on the modern tenant-scoped `companies`/`organizations`/`teams` tables; forward-apply runs alongside the legacy OU write, reverse-render sources from `HierarchyStore::get_hierarchy`. §2.2-C (PR #145) landed `roles` reverse-render via `render_roles` + LEFT-JOIN query against `user_roles × organizational_units`; tenant-wide sentinel (`unit_id == tenant_id`) maps to `unit: None`, scoped grants emit the OU name (falling back to raw id for orphaned rows). §2.2-D (this PR) landed `providers.memoryLayers` under the `memory_layer.{layer}.{field}` dotted-prefix convention. `NOT_RENDERED_SECTIONS` is now empty — the full structural diff can be implemented as a follow-up.)_
