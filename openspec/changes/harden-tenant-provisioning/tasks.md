...
- [ ] 2.4 Implement `POST /api/v1/admin/tenants/diff` returning structured delta between submitted manifest and current state. _(Sequenced after full-fidelity renderer: a diff is only trustworthy when every manifest section round-trips. §2.2-A landed `providers` (llm + embedding) forward-apply + reverse-render with secret_ref operator-name recovery. PR #128 landed `tenant.domainMappings` reverse-render and clarified top-level `secrets` as wire-only (not a gap). §2.2-D (PR #142) landed `providers.memoryLayers` under `memory_layer.{layer}.{field}` dotted-prefix convention. §2.2-B (PR #143 B3 + this branch B4) landed `hierarchy` via `HierarchyStore` on the modern tenant-scoped `companies`/`organizations`/`teams` tables; forward-apply runs alongside the legacy OU write, reverse-render sources from `HierarchyStore::get_hierarchy`. Remaining forward-path gap: `roles` only (§2.2-C). Diff ships after that closes.)_
