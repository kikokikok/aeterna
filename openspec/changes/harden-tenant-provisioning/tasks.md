...
- [x] 2.2 Implement `GET /api/v1/admin/tenants/{slug}/manifest` returning the rendered current-state manifest (secret values excluded; references only). _(Shipped in `cli/src/server/manifest_render.rs` + `manifest_api.rs`: MVP coverage of `tenant`, `metadata` (generation + hash), `config` (fields + secret_references), `repository`. Sections not yet reverse-rendered (`hierarchy`, `roles`, `providers`, `secrets`, `domainMappings`) are enumerated in the response's `notRendered: [...]` array so callers can distinguish "not rendered" from "empty". PA-gated via `authenticated_platform_context`.)_
...
- [ ] 2.4 Implement `POST /api/v1/admin/tenants/diff` returning structured delta between submitted manifest and current state. _(Blocked on full-fidelity renderer: a diff is only trustworthy when every manifest section round-trips. Waiting on reverse-rendering of `hierarchy`/`roles`/`providers` before shipping the diff surface, otherwise every diff would spuriously report "removed" for those sections.)_
