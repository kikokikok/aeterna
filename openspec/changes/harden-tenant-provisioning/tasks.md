...
- [ ] 2.4 Implement `POST /api/v1/admin/tenants/diff` returning structured delta between submitted manifest and current state. _(Sequenced after full-fidelity renderer: a diff is only trustworthy when every manifest section round-trips. §2.2-A landed `providers` (llm + embedding) forward-apply + reverse-render with secret_ref operator-name recovery. PR #128 landed `tenant.domainMappings` reverse-render and clarified top-level `secrets` as wire-only (not a gap). §2.2-B (PR #143 B3 + PR #148 B4) landed `hierarchy` via `HierarchyStore` on the modern tenant-scoped `companies`/`organizations`/`teams` tables. §2.2-C (this PR) landed `roles` reverse-render via `render_roles` + LEFT-JOIN query against `user_roles × organizational_units`; tenant-wide sentinel (`unit_id == tenant_id`) maps to `unit: None`, scoped grants emit the OU name (falling back to raw id for orphaned rows). Only remaining gap: `providers.memoryLayers` (PR #142, also pending merge). Diff unblocks once that lands.)_
