...
- Private deployment values (separate repo, not in public codebase)
...
The webhook secret is injected via `AETERNA_WEBHOOK_SECRET` env var (from K8s secret). The endpoint is exposed through the existing ingress — GitHub sends to `https://<your-aeterna-host>/api/v1/webhooks/github`.
...
Three new env vars:
- `AETERNA_KNOWLEDGE_REPO_URL`: SSH URL (e.g., `git@github.com:<your-org>/aeterna-knowledge.git`)
...
- `aeterna.knowledgeRepo.owner`: GitHub org/user (e.g., `<your-org>`)
...
2. **Deploy sequence**:
   ...
   i. Create GitHub repo `<your-org>/aeterna-knowledge` (empty, private)
