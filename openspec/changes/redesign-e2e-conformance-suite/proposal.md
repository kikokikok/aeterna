...
- **Portable to downstream consumers**: every deployment-specific value is an env var with a default; three orthogonal mode dials (`AETERNA_E2E_CLUSTER_MODE`, `AETERNA_E2E_SECRETS_BACKEND`, `AETERNA_E2E_GITHUB_MODE`) let an internal repo (e.g. a Kyriba-internal repo dedicated to the Kyriba production deployment) vendor `e2e/` and run the same conformance contract against its own aeterna instance, with its own secrets backend (`op` / `aws-sm` / `vault`), against `https://api.github.com` or a GitHub Enterprise URL, and either bringing up its own kind cluster or pointing at an existing one. CI templates for GitHub Actions and GitLab CI ship in `e2e/templates/ci/`; a reusable GitHub Actions workflow is also exported. Detail in `design.md` §D13.
...
- **Portability by design**: the suite is shipped so a downstream consumer (e.g. an internal Kyriba repo) can vendor `e2e/` (≈80 KB), populate four required env vars (`AETERNA_E2E_BASE_URL`, `AETERNA_E2E_PA_SIGNING_KEY`, `AETERNA_E2E_GITHUB_APP_ID`, `AETERNA_E2E_GITHUB_APP_KEY`), and run a green conformance gate against their own deployment. Verified by a CI smoke job that simulates the consumer-vendor path on every change to `e2e/`.
- **Out of scope (explicit non-goals)**: Okta SSO round-trip (deferred — needs a separate test tenant in Okta), browser-driven UI flows (Playwright already owns those), feature-coverage parity with the deleted folders (cargo/Playwright/§13 own that), publishing an OCI image of the suite (designed-for but tracked as a v2 follow-up).
