# GrepAI Repository Management: Local, Remote, and Incremental Indexing

## Problem Statement

The current GrepAI integration only supports **static local directories**. Real-world use cases require:
...
CREATE TABLE grepai_index_metadata (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    repository_id UUID NOT NULL REFERENCES grepai_repositories(id),
...
CREATE POLICY grepai_repositories_tenant_isolation 
ON grepai_repositories 
FOR ALL 
TO authenticated 
USING (tenant_id = current_setting('app.tenant_id', true)::UUID);

CREATE POLICY grepai_index_metadata_tenant_isolation 
ON grepai_index_metadata 
FOR ALL 
TO authenticated 
USING (
    repository_id IN (
        SELECT id FROM grepai_repositories 
...
# Remove a repository
aeterna grepai repo remove company-backend
# Output:
...
aeterna grepai repo update project  # Incremental only!
```
...
aeterna grepai repo status company-backend --verbose
...
curl -X POST http://localhost:9091/api/v1/grepai/webhook/github \
  -H "X-Hub-Signature-256: sha256=..." \
  -d @webhook-payload.json

# Check webhook logs
kubectl logs -n aeterna <pod> -c grepai --tail=100
```
