...
impl OpenCodeAdapter {
    ...
    pub fn get_codesearch_tools(&self) -> Vec<Value> {
        self.server
            .list_tools()
            .into_iter()
            .filter(|t| t.name.starts_with("codesearch_"))
            .map(|t| serde_json::to_value(t).unwrap())
            .collect()
    }
...
impl EcosystemAdapter for OpenCodeAdapter {
    ...
    async fn handle_mcp_request(&self, request: Value) -> anyhow::Result<Value> {
        ...
        if let Some(params) = request.get_mut("params")
            ...
            && args.get("tenant_id").is_none()
            && args.get("tenantContext").is_none()
            && let Ok(ctx) = self.resolve_implicit_context().await
            && let Ok(ctx_val) = serde_json::to_value(ctx)
        {
            args.as_object_mut()
                .unwrap()
        ...
        if let Some(params) = request.get("params")
            && let Some(tenant_id) = params.get("tenant_id")
